...
export type CircularProgressProps = {
  ...
  trackColor?: string;
  label?: string;
  /** Center caption (defaults to percentage). */
  caption?: string;
  className?: string;
};
...
export function CircularProgress({
  ...
  thickness = 8,
  color = 'var(--rd-accent)',
  trackColor = 'color-mix(in srgb, var(--rd-text-2) 25%, transparent)',
  label,
  caption,
  className = '',
  ...
  return (
    <div
      className={`rd-circular ${className}`.trim()}
    ...
    >
      ...
      {label && <span className="rd-circular__label">{label}</span>}
    </div>
  );
}
